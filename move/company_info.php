...
$stmt = $pdo->prepare("SELECT * FROM $tCompanies WHERE owner_username=? ORDER BY created_at DESC");
...
$reqStmt = $pdo->query("SELECT COUNT(*) FROM $tRequests WHERE status='Pending'");
...
<style>
.page-hero{padding:50px 24px 30px;text-align:center;color:white;position:relative;overflow:hidden;}
.page-hero::before{content:'';position:absolute;inset:0;background:url('https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=1920&q=80') center/cover no-repeat;filter:brightness(0.20) saturate(1.1);}
.page-hero::after{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(42,37,32,0.92) 0%,rgba(188,159,139,0.38) 100%);}
...
.add-btn{display:inline-flex;align-items:center;gap:8px;padding:11px 22px;background:linear-gradient(135deg,#BC9F8B,#a08070);color:white;border-radius:10px;text-decoration:none;font-weight:600;font-size:0.9rem;transition:var(--trans);}
.add-btn:hover{transform:translateY(-2px);box-shadow:0 6px 20px rgba(188,159,139,0.40);}
...
.c-logo-ph{width:100%;height:160px;background:linear-gradient(135deg,var(--linen,#E7E8D8),var(--mint,#CADABF));display:flex;align-items:center;justify-content:center;font-size:3.5rem;}
.c-body{padding:20px;}
...
.c-meta-item i{color:#BC9F8B;width:14px;}
...
.loc-tag{padding:3px 10px;background:rgba(202,218,191,0.25);color:#3a6b3c;border-radius:50px;font-size:0.75rem;font-weight:600;}
...
.status-pill.assigned{background:rgba(202,218,191,0.30);color:#3a6b3c;}
.status-pill.completed{background:rgba(181,207,183,0.40);color:#1e3a22;}
.status-pill.cancelled{background:rgba(231,232,216,0.80);color:#7a7060;}
  ...
  <div class="dash-header">
    <h2>Your Companies (<?= count($companies) ?>)
      <?php if ($pendingCount > 0): ?>
        <span style="background:rgba(188,159,139,0.18);color:#7a5a40;padding:3px 10px;border-radius:50px;font-size:0.75rem;margin-left:8px;"><?= $pendingCount ?> pending</span>
      <?php endif; ?>
    </h2>
  ...
  <?php else: ?>
    <div class="companies-grid">
      <?php foreach ($companies as $c):
        $svcStmt = $pdo->prepare("SELECT * FROM $tServices WHERE company_id=?");
        $svcStmt->execute([$c['id']]);
        $svcs = $svcStmt->fetchAll(PDO::FETCH_ASSOC);

        $reqStmt2 = $pdo->prepare("SELECT * FROM $tRequests WHERE company_id=? ORDER BY created_at DESC LIMIT 5");
        $reqStmt2->execute([$c['id']]);
        $requests = $reqStmt2->fetchAll(PDO::FETCH_ASSOC);
      ?>
      <div class="c-card">
        <?php if ($c['logo_path']): ?>
          <img src="<?= BASE ?>/move/<?= htmlspecialchars($c['logo_path']) ?>" class="c-logo" alt="Logo">
        <?php else: ?>
