...
if (!isset($_SESSION['loggedin'])) {
    header('Location: '.BASE.'/front/login.php?redirect='.urlencode(BASE.'/travel/add_vehicle.php'));
    exit;
}
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$err) {
        ...
        if (move_uploaded_file($_FILES['license_image']['tmp_name'], $licensePath) &&
            ...
            $stmt = $conn->prepare("INSERT INTO vehicles (owner_name,mobile,email,address,license_image,vehicle_image,capacity,rate_per_km,vehicle_category) VALUES (?,?,?,?,?,?,?,?,'travel')");
            $stmt->bind_param("ssssssdd", $name, $mobile, $email, $address, $licensePath, $vehiclePath, $capacity, $rate);

            if ($stmt->execute()) {
                header('Location: '.BASE.'/front/your_vehicle_travel.php?registered=1'); exit;
            } else {
...
<title>Add Travel Vehicle — Drifter</title>
<style>
.page-hero{
...
.form-card::before{content:'';position:absolute;top:0;left:0;width:5px;height:100%;background:linear-gradient(to bottom,#BC9F8B,#CADABF);}
...
.user-bar{background:rgba(202,218,191,0.20);border:1px solid rgba(181,207,183,0.45);border-radius:8px;padding:10px 14px;margin-bottom:20px;font-size:0.85rem;color:#3a6b3c;display:flex;align-items:center;gap:8px;}
...
.submit-btn{width:100%;padding:14px;background:linear-gradient(135deg,#BC9F8B,#a08070);color:white;border:none;border-radius:10px;font-size:1rem;font-weight:700;cursor:pointer;transition:all 0.25s;margin-top:8px;}
...
.b-icon{width:38px;height:38px;background:linear-gradient(135deg,#BC9F8B,#a08070);color:white;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:0.9rem;}
...
<body>
<div class="page-hero">
  <h1><i class="fas fa-bus"></i> Add Travel Vehicle</h1>
  <p>Register your travel vehicle and start receiving passenger bookings today</p>
</div>
<div class="container">
  <div class="form-card">
    <h2>Travel Vehicle Registration</h2>
    <?php if (!empty($err)): ?>
    ...
    <?php endif; ?>
    <div class="user-bar"><i class="fas fa-user-circle"></i> Registering as: <strong><?php echo htmlspecialchars($_SESSION['username']); ?></strong></div>
    <form id="regForm" action="add_vehicle.php" method="POST" enctype="multipart/form-data">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="number" name="capacity" placeholder="e.g. 6" min="1" step="1" required>
        </div>
        <div class="form-group">
          <label>Rate per KM (₹) *</label>
          <input type="number" name="rate_per_km" placeholder="e.g. 12" min="1" step="0.5" required>
        </div>
      </div>
      ...
      <div class="file-group">
        ...
        <label class="file-drop" id="vehDrop" for="vehicle_image">
          <i class="fas fa-bus"></i><span id="vehText">Click to upload vehicle photo</span>
        </label>
  ...
  <div class="benefits-card">
    <h3>Why Register?</h3>
    <div class="benefit"><div class="b-icon"><i class="fas fa-bolt"></i></div><div><h4>Instant Bookings</h4><p>Start receiving passenger bookings immediately.</p></div></div>
    <div class="benefit"><div class="b-icon"><i class="fas fa-shield-alt"></i></div><div><h4>Verified Passengers</h4><p>All passengers are registered users — safe trips.</p></div></div>
    <div class="benefit"><div class="b-icon"><i class="fas fa-rupee-sign"></i></div><div><h4>Set Your Rate</h4><p>You control your per-km rate.</p></div></div>
    <div class="benefit"><div class="b-icon"><i class="fas fa-chart-line"></i></div><div><h4>Track Bookings</h4><p>View all bookings from your dashboard.</p></div></div>
    <div class="benefit"><div class="b-icon"><i class="fas fa-toggle-on"></i></div><div><h4>Availability Control</h4><p>Toggle your vehicle on/off anytime.</p></div></div>
  </div>
</div>
