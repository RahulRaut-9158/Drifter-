...
$stmt = $conn->prepare("SELECT * FROM vehicles WHERE owner_name=? AND vehicle_category='travel' ORDER BY created_at DESC");
...
<title>Travel Dashboard — Drifter</title>
<style>
.dash-hero { background:linear-gradient(135deg,#0c4a6e 0%,#0891b2 100%); padding:48px 24px 36px; color:white; }
...
.btn-add { display:inline-flex; align-items:center; gap:8px; padding:11px 22px; background:linear-gradient(135deg,#06b6d4,#0891b2); color:white; border-radius:10px; font-weight:700; font-size:0.88rem; text-decoration:none; transition:transform 0.2s, box-shadow 0.2s; box-shadow:0 3px 12px rgba(6,182,212,0.40); }
.btn-add:hover { transform:translateY(-2px); box-shadow:0 8px 24px rgba(6,182,212,0.55); }
.btn-add-outline { display:inline-flex; align-items:center; gap:8px; padding:10px 20px; background:rgba(255,255,255,0.10); color:white; border-radius:10px; font-weight:600; font-size:0.88rem; text-decoration:none; border:1px solid rgba(255,255,255,0.25); transition:background 0.2s; }
...
.stat-card { background:white; border-radius:14px; padding:20px 22px; box-shadow:0 4px 20px rgba(15,23,42,0.10); display:flex; align-items:center; gap:16px; border-left:4px solid var(--c,#06b6d4); }
.stat-icon { width:46px; height:46px; border-radius:12px; background:var(--bg,#ecfeff); display:flex; align-items:center; justify-content:center; font-size:1.3rem; flex-shrink:0; }
...
.tab-btn.active { background:white; color:#0891b2; box-shadow:0 2px 8px rgba(15,23,42,0.10); }

.vehicles-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(340px,1fr)); gap:22px; }
.v-card { background:white; border-radius:18px; overflow:hidden; box-shadow:0 2px 16px rgba(15,23,42,0.08); transition:transform 0.28s, box-shadow 0.28s; border:1px solid #e2e8f0; }
...
.v-img-ph { width:100%; height:185px; background:linear-gradient(135deg,#ecfeff,#cffafe); display:flex; align-items:center; justify-content:center; font-size:3.5rem; }
...
.toggle-btn { width:100%; padding:10px; border:none; border-radius:9px; font-weight:700; font-size:0.84rem; cursor:pointer; transition:all 0.22s; margin-bottom:12px; font-family:inherit; display:flex; align-items:center; justify-content:center; gap:7px; }
...
.booking-row { display:grid; grid-template-columns:1fr auto auto auto; gap:8px; padding:10px 0; border-bottom:1px solid #f8fafc; font-size:0.78rem; align-items:center; }
...
<div class="dash-hero">
  <div class="dash-hero-inner">
    <div>
      ...
      <p>Welcome back, <?= htmlspecialchars($username) ?> — manage your travel vehicles &amp; passenger bookings</p>
    </div>
    <div class="hero-actions">
      <a href="<?= BASE ?>/front/your_vehicle_info.php" class="btn-add-outline"><i class="fas fa-truck"></i> Transport Vehicles</a>
      <a href="<?= BASE ?>/travel/add_vehicle.php" class="btn-add"><i class="fas fa-plus"></i> Add Vehicle</a>
    </div>
  </div>
</div>
...
<div class="dash-body">
  ...
  <div class="flash-ok"><i class="fas fa-check-circle"></i> Travel vehicle registered successfully!</div>
  <?php endif; ?>

  <div class="tab-nav">
    <button class="tab-btn active" onclick="showTab('vehicles',this)"><i class="fas fa-bus"></i> My Vehicles</button>
  ...
  <div id="tab-vehicles">
    <?php if (empty($vehicles)): ?>
      <div class="empty-state">
        ...
        <a href="<?= BASE ?>/travel/add_vehicle.php" class="btn-add"><i class="fas fa-plus"></i> Add Your First Vehicle</a>
      </div>
    <?php else: ?>
      <div class="vehicles-grid">
        ...
        <div class="v-card" id="vcard-<?= $vid ?>">
          <?php if ($v['vehicle_image']): ?>
            <img src="<?= BASE ?>/travel/<?= htmlspecialchars($v['vehicle_image']) ?>" class="v-img" alt="Vehicle">
          ...
          <div class="v-body">
            ...
            <div class="bookings-section">
              <div class="bookings-title"><i class="fas fa-users"></i> Recent Bookings (<?= count($bookings) ?>)</div>
  ...
  <div id="tab-bookings" style="display:none;">
    ...
    if ($vids) {
        $in = implode(',', array_fill(0, count($vids), '?'));
        $types = str_repeat('i', count($vids));
        $s = $conn->prepare("SELECT b.*, v.capacity FROM booking b JOIN vehicles v ON b.vehicle_id=v.id WHERE b.vehicle_id IN ($in) ORDER BY b.id DESC");
        $s->bind_param($types, ...$vids); $s->execute();
    ...
    <div class="bookings-table-wrap">
      <div class="table-header">
        <h3><i class="fas fa-list"></i> All Travel Bookings (<?= count($allBookings) ?>)</h3>
      </div>
      ...
      <div style="overflow-x:auto;">
        <table>
          <thead>
            <tr><th>#</th><th>Passenger</th><th>Route</th><th>Date</th><th>Distance</th><th>Amount</th><th>Status</th><th>Action</th></tr>
          </thead>
          <tbody>
